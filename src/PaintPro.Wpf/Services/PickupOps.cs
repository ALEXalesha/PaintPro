...
public static class PickupOps
{
    ...
    public static void EnsureLazyErase(Document doc, FloatingPickup fp)
    {
        ...
        // Снимок слоя пересним ПРЯМО СЕЙЧАС, перед тем как выкусить из него исходную
        // область. Он описывает «слой до стирания», и именно этим он нужен и Escape
        // (<see cref="Models.Document.CancelFloating"/>), и записи прижатия
        // (<see cref="Models.Document.CommitFloating"/>).
        //
        // Снятый при подъёме, он описывал слой на момент подъёма, а между подъёмом и
        // первым движением слой успевает измениться: подъём холста не трогает, и до
        // первого сдвига по нему можно рисовать. Такая правка потом пропадала - её
        // затирало «до» из устаревшего снимка при первом же Ctrl+Z. Нашёл это фаззер:
        // жест выделения оставляет объект в руках, и следующая правка ложится в слой
        // под ним.
        //
        // У вставки снимка нет и не будет: она ничего с холста не поднимала.
        if (fp.PreEditSnapshot is not null)
        {
            var fresh = pl.ExtractRegion(new SKRectI(0, 0, pl.Width, pl.Height));
            fp.PreEditSnapshot.Dispose();
            fp.PreEditSnapshot = fresh;
        }
    ...
    /// <summary>
    /// Сторона квадратной ручки масштабирования на экране.
    ///
    /// То же правило, что у зоны хвата угла (<see cref="HitCorner"/>), и по той же
    /// причине: ручка не может быть больше того, за что она держит. Восемь ручек по
    /// <paramref name="preferred"/> пикселей стоят по углам и серединам сторон, и у
    /// объекта меньше двух таких размеров верхняя и нижняя ручки смыкаются посередине -
    /// накрывают его целиком. Тело после этого не ухватить вовсе: клик в любую точку
    /// объекта попадает в ручку, и вместо перетаскивания начинается масштабирование.
    /// Начиналось это на 24 экранных пикселях, то есть на любом выделении с ноготь при
    /// масштабе 1:1 и на картинке в четверть натуральной величины - там 24 экранных
    /// пикселя это почти сто пикселей документа.
    ///
    /// Половина меньшей стороны оставляет середину телу при любом размере и не трогает
    /// ручки на крупных объектах: там своя половина заведомо больше <paramref
    /// name="preferred"/>. Нижняя граница в три пикселя - чтобы у совсем мелкого объекта
    /// ручки не исчезли совсем.
    /// </summary>
    public static double HandleSize(FloatingPickup fp, double zoom, double preferred)
    {
        double side = Math.Min(fp.Width, fp.Height) * Math.Max(zoom, 0.01);
        return Math.Max(3, Math.Min(preferred, side / 2));
    }
