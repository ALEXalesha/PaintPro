...
- `tests/PaintPro.Tests/`: 840 xUnit tests.
- `installer/PaintPro.iss`: Inno Setup.
- `tools/PaintPro.Screenshots/`: the README frames.

The full design, with the seven anti-patterns of the original Electron code and how each is handled here, is in [`REWRITE_PROMPT_CSHARP.md`](../REWRITE_PROMPT_CSHARP.md). The running list of invariants (eighty-four so far, most with the bug that produced them) is in the [Russian README](CSHARP.ru.md#ключевые-инварианты).
