# Устройство Paint Pro

Этот файл - про то, как приложение собрано изнутри и какие правила его держат. Что оно
умеет, как его поставить и запустить - в [README](../README.ru.md); здесь начинается
там, где README заканчивается.

Половина разделов ниже - разбор конкретных граблей: почему сохранение идёт двумя шагами
IPC, почему прозрачность накладывается один раз на мазок, откуда взялись потолки ленты
истории. Каждый такой раздел написан после того, как соответствующее правило было
нарушено и это сломало работу.

## Что даёт оболочка Electron
