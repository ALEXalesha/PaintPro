...
37. **Событие «курсор ушёл» на элементе, поверх которого что-то лежит.** У `<canvas>`
    `mouseleave` приходит не только когда курсор ушёл за границы, но и когда над ним
    оказался ПЕРЕКРЫВАЮЩИЙ элемент: рамка выделения и её ручки лежат отдельными `div`, а
    не детьми холста. Пока на этом событии висело завершение жеста, ресайз срывался от
    отклонения в пару пикселей - курсор соскакивал с ручки на холст и обратно, - а мазок
    обрывался на краю холста и не продолжался при возврате. Жест обязан кончаться
    отпусканием кнопки. Обратная сторона: отпускание за пределами окна не приходит вовсе,
    поэтому зависший жест нужно подбирать явно - первым движением с уже отпущенной
    кнопкой (`e.buttons === 0`) и потерей фокуса окном.

38. **Один и тот же список, набранный руками в двух местах.** Перечень фигур жил и в
    `SHAPE_TOOLS`, и отдельно в памяти размера кисти. Новый инструмент попадал в панель, а
    во второй перечень - нет, и размер при переключении на него залипал от ластика. Списки
    такого рода обязаны выводиться один из другого.

39. **Ожидание анимации по таймеру в проверках.** Проверки читаемости тем ждали конца
    перехода фиксированные 250 мс. На загруженной машине кадры приходят реже, и
    `getComputedStyle` отдавал цвет ПОСРЕДИ перехода - падение на цвете, которого
    пользователь не увидит. Ждать «пока устоится» мало: замерять пришлось бы ровно те
    элементы, которые прочтёт проверка. Переходы на время таких проверок выключаются
    целиком: они про плавность, а проверка - про конечные цвета.
