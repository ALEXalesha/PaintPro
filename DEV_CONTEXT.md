...
Если у истории есть потолок и она вытесняет старые записи, то всё, что ссылается
на позицию в ней (метка «здесь сохранено», закладки, «вернуться сюда»), обязано
жить рядом с самим списком и сдвигаться в той же функции. Копия индекса, лежащая
в другом объекте, после первого же вытеснения начинает указывать не туда — и не
падает, а тихо врёт. См. антипаттерн 26.
...
21. **Считать «пользователь что-то сделал» по факту вызова, а не по результату.**
    ...
    Второй случай того же нашёлся позже в заливке: `floodFill` выходила сразу, если
    под курсором уже нужный цвет, а вызывающий код всё равно писал запись. Правило
    работает только если операция сама возвращает «изменил / не изменил»; пока это
    решает вызывающая сторона, у каждой операции свой шанс забыть.
...
26. **Индекс в список, который сам себя обрезает, хранить снаружи этого списка.**
    У истории есть потолок по глубине и по памяти, и при переполнении она
    выбрасывает старые записи, сдвигая курсор вниз. Метка «на этой позиции файл был
    сохранён» лежала отдельным полем в другом объекте и не сдвигалась. Пока
    история короткая, всё сходится; после переполнения курсор перестаёт расти —
    каждая новая правка вытесняет одну старую и возвращает его на тот же потолок, —
    и сравнение «курсор ≠ метка» навсегда даёт «сохранено». Приложение закрывалось
    без вопроса и уносило всё нарисованное после переполнения. Ошибка тихая вдвойне:
    она не проявляется ни на одном коротком сценарии и включается ровно тогда, когда
    работы накопилось больше всего. Держать такие метки внутри той же структуры и
    сдвигать в той же функции, что и курсор; наружу отдавать не число, а ответ
    (`isDirtySinceSave`), чтобы вызывающей стороне нечего было сравнивать самой.

27. **Захват мыши на элементе, который перерисовка пересоздаёт.** Ручки
    масштабирования перерисовывались каждый кадр: слой оверлея чистился целиком и
    наполнялся заново. Элемент, за который тянули, при этом уходил из дерева, а
    вместе с ним терялся захват мыши, и остаток жеста доставался тому, кто оказался
    под курсором. Заметно это не сразу: у угловых ручек новая встаёт ровно под
    курсором, и жест продолжается по случайности; у боковых, которые едут только по
    одной оси, движение поперёк на несколько пикселей обрывает перетаскивание, и
    дальше события уходят на холст активному инструменту. Захват брать на
    контейнере, который переживает перерисовку, и слушать движение на нём же.
    Общее правило: время жизни элемента должно быть не меньше времени жизни жеста,
    который на него завязан.

28. **Ошибка в другом процессе, отданная отклонением промиса.** В Electron
    `ipcRenderer.invoke` отклоняется, если обработчик в main бросил исключение. В
    renderer результат почти везде ждут без `catch`, поэтому для пользователя это
    выглядит как «нажал Открыть, выбрал файл, ничего не произошло»: ни картинки, ни
    сообщения, только строка в консоли, которую никто не смотрит. Обработчики,
    трогающие внешний мир, ловят ошибку сами и возвращают её значением
    (`{ error }`), а вызывающая сторона показывает. Проверять это стоит по списку
    обработчиков, а не по памяти: у нас из трёх читающих файл обработчиков
    `try/catch` был в двух, и третий полгода молча глотал ошибки.

29. **Позиция в списке как идентификатор состояния.** Метка «здесь сохранено» -
    номер шага в истории. Новая правка после отмены сначала срезает хвост повтора, и
    курсор встаёт на тот же номер, где стояла метка: «сохранили, отменили,
    нарисовали заново» давало документ, который приложение считало сохранённым, при
    другом содержимом файла. Окно закрывалось без вопроса. Индекс отвечает на вопрос
    «сколько», а не «что»: если элементы под ним могут смениться, метка обязана
    инвалидироваться в той же функции, которая их меняет.

30. **Открыть файл, не решив, что такое «открыть».** Картинка приходила плавающим
    объектом поверх текущего рисунка - то есть как вставка, - но путь к файлу при
    этом уже становился целью следующего сохранения. Два разных решения в двух
    местах, и между ними пользовательская фотография, переписанная композитом из неё
    и чужих штрихов. Замена документа и вставка содержимого - разные операции с
    разными последствиями для цели сохранения; выбирать между ними должен один код,
    общий для всех источников (диалог, ассоциация файлов, drag & drop).

31. **Состояние, влияющее на результат, но живущее мимо истории.** Видимость и
    прозрачность слоя писались прямо в модель: отмена их не возвращала, а признак
    несохранённой работы, считающийся по позиции в истории, их не видел - при том,
    что в сохраняемую картинку они попадают. Правило простое: всё, что меняет
    итоговый файл, проходит через ту же историю, что и штрихи. Ползунок при этом
    шлёт значение на каждый пиксель перетаскивания, поэтому у команды должна быть
    склейка с предыдущей такой же, иначе один жест даёт полсотни записей.


32. **Склейка правок в обход того, что делает обычная запись.** Подряд идущие
    движения ползунка дописываются в последнюю запись истории вместо новой - иначе
    один жест даёт полсотни строк. Но «дописать» проходит мимо всего, что делает
    обычное добавление: курсор не двигается, хвост повтора не срезается, метка
    сохранения не инвалидируется. «Выключили слой, сохранили, включили обратно»
    возвращало документ в состояние, которое приложение считало сохранённым.
    Оптимизация, срезающая шаг, обязана сама сделать всё, что делал срезанный шаг;
    у склейки для этого должен быть свой вход в структуру (`AmendCurrent`), а не
    прямая запись в объект мимо неё. И склеивать только то, что действительно один
    жест: одно свойство и соседние по времени правки, а не «последняя запись про
    тот же объект» - иначе в одну строку попадают правки, сделанные с разницей в час.

33. **Правило, записанное в документации, но не в коде.** Метка сохранения,
    попавшая в срезаемый хвост повтора (см. п. 29), была описана в README
    Electron-версии как реализованная - и не была реализована. Правку получила
    только WPF-версия, а запись в общий CHANGELOG была сделана про обе. Две
    параллельные реализации одного продукта расходятся именно так: правку делают в
    той, где баг нашли, а в тексте описывают поведение продукта. Чинить в обеих в
    том же заходе, а если во второй нельзя - писать в CHANGELOG, что там это ещё не
    сделано. Документация не проверяется компилятором.

34. **Действие, которое уходит в невидимое место.** Штрих по скрытому слою попадал
    в его битмап целиком: на экране не менялось ничего, зато в истории появлялась
    запись, а нарисованное всплывало в сохранённом файле, стоило слой включить.
    Пользователь при этом видит, что инструмент не работает, и не знает почему.
    Если у операции есть состояние, при котором она бессмысленна, проверять его
    надо в одном месте на все инструменты (`ToolContext.DrawTarget()`) и отказывать
    не молча: причина уходит в статусбар. Молчаливый отказ неотличим от поломки.

35. **Разделяемый системный ресурс, который бывает занят.** Буфер обмена в Windows
    захватывается монопольно, и пока его держит чужое приложение, вызов падает с
    `COMException` - на обычном Ctrl+C, без всякой вины пользователя. Такие отказы
    нормальны и лечатся несколькими попытками с паузой, а не исключением, всплывшим
    до глобального обработчика. И если копия не удалась, вырезание не должно
    продолжаться: иначе пиксели уходят в никуда.

36. **Асинхронное восстановление состояния без замка.** Кадр истории грузится через
    `img.onload`, то есть после текущей задачи. Между Ctrl+Z и появлением картинки
    на холсте лежит состояние ДО отмены, и всё, что читает холст в это окно
    (сохранение) или пишет в него (новый штрих), работает не с тем, что пользователь
    уже видит. Окно короткое, руками попадают редко, а программно - постоянно.
    Асинхронная замена состояния должна отдавать обещание, которого можно дождаться,
    и запирать вход на время; снимать замок при этом имеет право только самый свежий
    запрос, иначе обогнанный кадр открывает дверь посреди следующего.
