...
- `paint-pro-electron/` — исходная Electron-версия, живёт параллельно и
  ...
  `CHANGELOG.md` (обе версии — 1.10.0).
...
22. **Действие, не изменившее ни пикселя, не пишется в историю** — подъём
    ...
    себе поднятый пикап не считается несохранённой работой: `IsDirty` смотрит на
    `OriginalAreaErased` — тот же флаг, по которому решает, писать ли в историю,
    `CommitFloating`. В Electron это `floatingMoved()` внутри `isDirty()`.
...
31. **Инструменты пишут только в видимый слой** — единственная проверка живёт в
    `ToolContext.DrawTarget()`, через неё идут кисти, фигуры, заливка и текст.
    Штрих по скрытому слою уходил в его битмап целиком: на экране не появлялось
    ничего, в истории появлялась запись, а нарисованное всплывало в файле, стоило
    слой включить. Отказ не молчит — причина уходит в статусбар через
